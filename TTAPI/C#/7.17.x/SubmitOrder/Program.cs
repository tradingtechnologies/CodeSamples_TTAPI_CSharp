    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            using (var disp = Dispatcher.AttachUIDispatcher())
            {
                ...
                frmSubmitOrder submitOrder = new frmSubmitOrder();
                ApiInitializeHandler handler = new ApiInitializeHandler(submitOrder.ttApiInitHandler);
                TTAPI.CreateXTraderModeTTAPI(disp, handler);

                Application.Run(submitOrder);
            }
        }
    }
}