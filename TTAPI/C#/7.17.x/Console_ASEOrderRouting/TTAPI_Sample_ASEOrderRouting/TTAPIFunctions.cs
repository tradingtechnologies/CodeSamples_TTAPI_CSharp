...
namespace TTAPI_Sample_ASEOrderRouting
{
    ...
    class TTAPIFunctions : IDisposable
    {
        ...
        public void Init()
        {
            ...
            TTAPI.CreateUniversalLoginTTAPI(Dispatcher.Current, "JSMITH", "12345678", h);
        }
        ...
        private void CreateSpreadDetails()
        {
            ...
            foreach (Instrument instrument in spreadLegKeys.Values)
            {
                SpreadLegDetails spreadlegDetails = new SpreadLegDetails(instrument);
        ...
        private void StartOrderSubmission(AutospreaderInstrument instrument)
        {
            ...
            AutospreaderSyntheticOrderProfile profile = new AutospreaderSyntheticOrderProfile(instrument.GetValidGateways()["TT"], instrument);
            profile.QuantityToWork = Quantity.FromInt(instrument, 1);
            profile.OrderType = OrderType.Limit;
            profile.BuySell = BuySell.Buy;
            profile.LimitPrice = Price.FromTick(instrument, 0);
        ...
        public void ts_OrderUpdated(object sender, OrderUpdatedEventArgs e)
        {
            if (e.OldOrder.SiteOrderKey == orderKey)
            {
                ...
                Console.WriteLine("Our parent order (" + orderKey + ") has been updated: " + e.Message);
            }
            else if (e.OldOrder.SyntheticOrderKey == orderKey)
            {
                // A child order of our parent order has been updated
                Console.WriteLine("A child order (" + e.OldOrder.SiteOrderKey + ") of our parent order (" + orderKey + ") has been updated: " + e.Message);
            }
        }
        ...
        {
            Console.WriteLine(e.Order.FeedConnectionKey.TraderId);
            if (e.Order.SiteOrderKey == orderKey)
            {
                // Our parent order has been added
                Console.WriteLine("Our parent order (" + orderKey + ") has been added: " + e.Message);
            }
            else if (e.Order.SyntheticOrderKey == orderKey)
            {
                // A child order of our parent order has been added
                Console.WriteLine("A child order (" + e.Order.SiteOrderKey + ") of our parent order (" + orderKey + ") has been added: " + e.Message);
            }
        }
    }
}
